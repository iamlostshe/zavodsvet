	...
	function Y(a, c, f) {
		...
		function z(a, b, c) {
			...
			return "positions" === a
				...
				? c
					? b.map(function (a) {
							return va.fromStepping(
								va.getStep(va.toStepping(a))
							);
					  })
					: b
				: void 0;
...
$(function () {
		...
		$(this).request("categoryFilter::onSetFilter", {
			...
			complete: function (response) {
				...
				if (response.responseJSON.hasOwnProperty("filter")) {
					for (var filter of Object.keys(
						response.responseJSON.filter
					)) {
	...
	$("html").click(function (e) {
		if (
			$(e.target).closest(
				".TitlePage_categoriya.open .TitlePage_categoriyaBTN"
			).length == 0
		) {
			$(".TitlePage_categoriya.open").removeClass("open");
		}
...
function resizeFunc(){
  ...
  $(".slick-ManePage__item_1").on("mousemove", (e) => {
		...
		document.getElementById("slide1_0").src = "images/slides/slaid1/"+(index)+".jpg"
	});
}
	...
	if (typeof pdoPage == "undefined") {
		...
		$.post(
			...
			function (response) {
				...
				if (
					typeof getUrlParams(window.location.href).page !=
					"undefined"
				) {
					let pageStr =
